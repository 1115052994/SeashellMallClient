...
public class SlidingTextTabLayout extends HorizontalScrollView implements ViewPager.OnPageChangeListener {
    ...
    private static final float TEXT_SELECT_SCALE = 0.2f;
    ...
    //打开滑动缩放
    private boolean mOpenScan = true;
    ...
    public SlidingTextTabLayout(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public SlidingTextTabLayout(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void obtainAttributes(Context context, AttributeSet attrs) {
        ...
        mIndicatorCornerRadius = ta.getDimension(R.styleable.SlidingTabLayout_tl_indicator_corner_radius, dp2px(mIndicatorStyle == STYLE_BLOCK ? -1 : 4));
        ...
        mIndicatorMarginBottom = ta.getDimension(R.styleable.SlidingTabLayout_tl_indicator_margin_bottom, dp2px(mIndicatorStyle == STYLE_BLOCK ? 7 : 2));
        ...
        if (AppConfigCache.get().getAppTheme().getColor_main() != 0) {
            if (mTextSelectColor != Color.parseColor("#ffffff")) {
                mTextSelectColor = mContext.getResources().getColor(AppConfigCache.get().getAppTheme().getColor_main());
            }
            if (mIndicatorColor != Color.parseColor("#ffffff")) {
                mIndicatorColor = mContext.getResources().getColor(AppConfigCache.get().getAppTheme().getColor_main());
            }
        }
    ...
    private void addTab(final int position, String title, View tabView) {
        TextView tv_tab_title = getTitleView(tabView);
        if (tv_tab_title != null) {
            if (title != null) {
                if (title.contains("\n")) {
                    SpannableString spannableTitle = new SpannableString(title);
                    spannableTitle.setSpan(new RelativeSizeSpan(0.7f), title.indexOf("\n") + 1, title.length(), Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
                    tv_tab_title.setText(spannableTitle);
                } else {
...
//            v.setPadding((int) mTabPadding, v.getPaddingTop(), (int) mTabPadding, v.getPaddingBottom());
            TextView tv_tab_title = getTitleView(v);
            if (tv_tab_title != null) {
                tv_tab_title.setTextColor(i == mCurrentTab ? mTextSelectColor : mTextUnselectColor);
//                setScale(tv_tab_title, i == mCurrentTab);
    ...
    public void onPageScrolled(int position, float positionOffset, int positionOffsetPixels) {
        ...
        scrollToCurrentTab();
        if (mOpenScan) {
            setScale(mCurrentTab, TEXT_SELECT_SCALE * (1 - positionOffset));
            if (mCurrentTab + 1 < mTabCount) {
                setScale(mCurrentTab + 1, TEXT_SELECT_SCALE * positionOffset);
            }
        }
        invalidate();
    }
    ...
    private void updateTabSelection(int position) {
        for (int i = 0; i < mTabCount; ++i) {
            ...
            TextView tab_title = getTitleView(tabView);

            if (tab_title != null) {
                tab_title.setTextColor(isSelect ? mTextSelectColor : mTextUnselectColor);
//                setScale(tab_title, isSelect);
    ...
    private void setScale(TextView tab_title, boolean isSelect) {
        tab_title.setScaleX(1 + (isSelect ? TEXT_SELECT_SCALE : 0));
        tab_title.setScaleY(1 + (isSelect ? TEXT_SELECT_SCALE : 0));
    }

    private void setScale(int position, float offset) {
        TextView tab_title = getTitleView(position);
        tab_title.setScaleX(1 + offset);
        tab_title.setScaleY(1 + offset);
        if (mInitSetMap.get(position) != null && mInitSetMap.get(position)) {
            setMsgMargin(position, 8, 2, 1 + offset);
        }
    }
    ...
    private void calcIndicatorRect() {
        ...
        if (mIndicatorStyle == STYLE_NORMAL && mIndicatorWidthEqualTitle) {
            TextView tab_title = getTitleView(currentTabView);
        ...
        if (this.mCurrentTab < mTabCount - 1) {
            ...
            //for mIndicatorWidthEqualTitle
            if (mIndicatorStyle == STYLE_NORMAL && mIndicatorWidthEqualTitle) {
                TextView next_tab_title = getTitleView(nextTabView);
                mTextPaint.setTextSize(mTextsize);
    ...
    public void setOpenScan(boolean mOpenScan) {
        this.mOpenScan = mOpenScan;
    }
    ...
    public TextView getTitleView(int tab) {
        ...
        tv_tab_title.setSingleLine(false);
        return tv_tab_title;
    }

    public TextView getTitleView(View tabView) {
        TextView tv_tab_title = (TextView) tabView.findViewById(R.id.tv_tab_title);
        tv_tab_title.setSingleLine(false);
        return tv_tab_title;
    }
    ...
    public void showMsg(int position, int num) {
        ...
        if (tipView != null) {
            ...
            setMsgMargin(position, 8, 2, 1 + (position == mCurrentTab ? 0.2f : 0f));
     ...
     */
    public void setMsgMargin(int position, float leftPadding, float bottomPadding, float scale) {
        if (position >= mTabCount) {
            position = mTabCount - 1;
        }
        View tabView = mTabsContainer.getChildAt(position);
        ...
        if (tipView != null) {
            TextView tv_tab_title = getTitleView(tabView);
            mTextPaint.setTextSize(mTextsize);
            float textWidth = mTextPaint.measureText(tv_tab_title.getText().toString()) * scale;
            float textHeight = (mTextPaint.descent() - mTextPaint.ascent()) * scale;
    ...
    public void showNum(int position,int num){
        ...
        View tabView = mTabsContainer.getChildAt(position);
        TextView tv_tab_title = (TextView) tabView.findViewById(com.netmi.baselibrary.R.id.tv_tab_title);
        tv_tab_title.setText(mTitles.get(position)+" （"+num+"）");
