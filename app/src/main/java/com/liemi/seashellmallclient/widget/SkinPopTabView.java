...
/*************************************************/
     ...
     * @param filterIndex
     */
    public void setDefaultCheckedItems(int filterIndex) {
        this.currentIndex = filterIndex;
        mViewLists.get(filterIndex).getAdapter().clearChecked();
    }
     ...
     * @return view 本身
     */
    public SkinPopTabView addFilterItem(String title, List data, int filterType, int singleOrMutiply) {

        ////默认筛选项的布局,如果想修改筛选项样式,也可以在此布局修改
        View labView = inflate(getContext(), R.layout.sharemall_item_pops_tab_view, null);
        ...
        if (currentIndex == mTextViewLists.size()) {
            labButton.setTextColor(choiceColor());
        }
        ...
        labView.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                //隐藏软键盘，否则popwindow弹出位置不准确
                KeyboardUtils.hideKeyboard(labView);
    ...
    private void setMenuDrawble(TextView tv_checked, boolean isUp) {
        ...
        if (isUp) {
            ...
            if (tab_text_color_focus != -1) {
                tv_checked.setTextColor(choiceColor());
            } else {
                tv_checked.setTextColor(ContextCompat.getColor(getContext(), R.color.bgColor));
            }
            tv_checked.setCompoundDrawablesWithIntrinsicBounds(0, 0, R.mipmap.sharemall_filter_up, 0);
            ...
            //没有选择内容时
            if (mTextViewLists.indexOf(tv_checked) == currentIndex && !hasChoiceItem(currentIndex)) {
                tv_checked.setTextColor(getResources().getColor(R.color.gray_99));
                tv_checked.setCompoundDrawablesWithIntrinsicBounds(0, 0, R.mipmap.sharemall_ic_sort_price, 0);
                return;
            }

            if (tab_text_color_normal != -1) {

                if (mTextViewLists.size() > currentIndex && mTextViewLists.get(currentIndex) == tv_checked) {
                    tv_checked.setTextColor(choiceColor());
                } else {
            ...
            tv_checked.setCompoundDrawablesWithIntrinsicBounds(0, 0, R.mipmap.sharemall_filter_down, 0);

        }
    }

    private boolean hasChoiceItem(int position) {
        if (mViewLists.size() > position) {
            SuperAdapter adapter = mViewLists.get(position).getAdapter();
            if (adapter != null && !adapter.getCheckedLists().isEmpty()) {
                return adapter.getCheckedLists().get(0) > -1;
            }
        }
        return false;
    }
    ...
    private void show(int position){
        //修复存在软键盘时，popWindow位置显示错误问题
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
            int[] location = new int[2];
            getLocationOnScreen(location);
            int tempHeight = mViewLists.get(position).getHeight();
            if (tempHeight == WindowManager.LayoutParams.MATCH_PARENT || ScreenUtils.getScreenHeight() <= tempHeight) {
                mViewLists.get(position).setHeight(ScreenUtils.getScreenHeight() - location[1] - getHeight());
            }
            mViewLists.get(position).showAtLocation(this, Gravity.NO_GRAVITY, location[0], location[1] + getHeight());
            mViewLists.get(position).update();
        } else {
            if (mViewLists.get(position) != null) {
...
/*****************************筛选成功,回调~************************************/
    ...
    public SkinPopTabView setOnPopTabSetListener(OnPopTabSetListener onPopTabSetListener) {
    ...
    public SkinPopTabView(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context, attrs);
    }

    public SkinPopTabView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    public SkinPopTabView setResultLoader(ResultLoader resultLoader) {
    ...
    public SkinPopTabView setPopEntityLoader(PopEntityLoader popEntityLoader) {
